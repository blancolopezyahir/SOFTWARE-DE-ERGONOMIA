...
Si el archivo no contiene `SkinnedMesh`, `Skeleton` y `Bones`, se muestra el error: `El modelo GLB no contiene un rig/skinning utilizable para deformación. Debe exportarse nuevamente desde Blender con Armature y Skinning.` y se activa el fallback CSS. Para corregir nombres reales, copia los nombres de la consola a `explicitBones` en `src/config/humanModelConfig.ts`. Para corregir ejes o signos anatómicos, edita `joints` en ese mismo archivo.

## Cargar GLB sin copiarlo al repositorio

Si no puedes colocar el archivo físicamente en `public/models/`, abre la app con `npm run dev` y usa el botón **Cargar GLB** dentro del visor 3D. Ese selector carga un `.glb` local del navegador mediante `URL.createObjectURL(...)` y ejecuta el mismo diagnóstico de rig, `SkinnedMesh`, `Skeleton`, huesos mapeados y sliders. Esta carga es por sesión del navegador; para dejarlo permanente en el proyecto, el archivo debe estar en `public/models/final low poly character rigged.glb`.
